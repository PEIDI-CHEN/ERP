...
namespace Common
{
    class LoadLayout
    {
        ...
        public string SetLayout()
        {
            ...
            {
                MessageBox.Show(ex.Message, "XML错误1");
            }
            return baseClass;
        }
        ...
        private object ReadObject(XmlNode node)
        {
            ...
            if (typeAttr == null)
            {
               MessageBox.Show("<Object> tag is missing required type attribute","错误6");
                return null;
            }
            ...
            if (type == null)
            {
                MessageBox.Show(string.Format("Type {0} could not be loaded.", typeAttr.Value),"错误7");
                return null;
            }
