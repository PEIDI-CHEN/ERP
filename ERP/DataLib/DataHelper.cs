...
namespace DataLib
{
    public class DataHelper
    {
        ...
        public DataSet GetDs(string strSQL)
        {
            ...
            DataSet ds = null;
            byte[] bUserData = null;
            if (DataLib.SysVar.intConnect == 0)////连接标识(0:webservice;1:remoting)
            {
                JxcService.Service myService = GetService();
                bUserData = myService.GetCompressDataSet(ref err, SysVar.strDB, strSQL);
                if (err != "")
                {
                    MessageBox.Show(err, "错误55");
                    err = "";
                    return null;
                }
                ds = DataSetCompression.DecompressionDataSet(bUserData);//序列化
                return ds;
            }
            else
            {
                CenterLib.Service obj = GetObj();//连接标识(0:webservice;1:remoting)

                bUserData = obj.GetCompressDataSet(ref err, SysVar.strDB, strSQL);
                if (err != "")
                {
                    MessageBox.Show(err, "错误21");
                    err = "";
                    return null;
                }
